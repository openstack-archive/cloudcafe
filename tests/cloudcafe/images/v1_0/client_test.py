...
class ClientTest(TestCase):
      ...
      if method.__name__ is 'test_retrieve_image_metadata' or \
         ...
         method.__name__ is 'test_update_image':
          self.expected_headers = {
              'x-image-meta-uri'              : '{0}/images/1c675abd94f49cda114e12490b328d9'.format(GLANCE_API_SERVER_ENDPOINT),
  ...
  def test_filter_images_list(self):
    ...
    HTTPretty.register_uri(HTTPretty.GET, "{0}/images?status=active&name=precise&container_format=bare&disk_format=qcow2&min_disk=0&size=252116992".format(GLANCE_API_SERVER_ENDPOINT),
      body=self._build_response_body(),
      content_type="application/json")

    actual_response = self.images_client.filter_images_list(filtering_parameters)
    request_querystring = HTTPretty.last_request.querystring
    expected_request_querystring = self._get_querystring_data(request_querystring)
  ...
  def test_add_image(self):
    HTTPretty.register_uri(HTTPretty.POST, self.images_uri,
      ...
      adding_headers= {'x-image-meta-property-distro': 'Ubuntu 10.04 LTS'})

    image_name = 'Ubuntu 10.04 Plain 5GB'
    actual_response = self.images_client.add_image(image_name=image_name, image_data=None)

    assert 'x-image-meta-name' in HTTPretty.last_request.headers.keys()

    assert 200 == actual_response.status_code
    assert self.images_uri == actual_response.url
    assert 'Adding New Image' == actual_response.content
    assert 'x-image-meta-property-distro' in actual_response.__dict__['headers'].keys()

  def test_retrieve_image_metadata(self):
    url = self.images_uri + '71c675ab-d94f-49cd-a114-e12490b328d9'
    HTTPretty.register_uri(HTTPretty.HEAD,
            url,
            body='Raw Image Data', headers=self.expected_headers)

    actual_response = self.images_client.retrieve_metadata('71c675ab-d94f-49cd-a114-e12490b328d9')

    assert re.match(r'http://localhost:9292/v1/images/[\w\d]{8}-[\w\d]{4}-[\w\d]{4}-[\w\d]{4}-[\w\d]{12}', \
            actual_response.url) is not None
    assert HTTPretty.last_request.headers['X-Auth-Token'] == '36a04b4e71484ab9aacb1d0ac95733fc'
    assert HTTPretty.last_request.headers['Content-Type'] == 'application/json'
    assert HTTPretty.last_request.headers['Accept'] == 'application/json'
    assert 200 == actual_response.status_code
  ...
  def test_retrieve_raw_image_data(self):
      ...
      actual_response = self.images_client.retrieve_raw_image_data('71c675ab-d94f-49cd-a114-e12490b328d9')
  ...
  def test_list_image_memberships(self):
    ...
    actual_response = self.images_client.list_image_membership(self.image_id)
  ...
  def _get_querystring_data(self, querystring):
      ...
      for key in querystring:
          querystring_data[key] = querystring[key][0] or int(querystring[key][0])
  ...
  def test_update_image(self):
      ...
      actual_response = self.images_client.update_image('71c675ab-d94f-49cd-a114-e12490b328d9',
  ...
  def test_list_shared_images(self):
      member_id = ''
      shared_images_url = GLANCE_API_SERVER_ENDPOINT + '/shared_images/{0}'.format(member_id)
  ...
  def test_add_member_to_an_image(self):
      ...
      actual_response = self.images_client.add_member_to_image(self.image_id, member_id)
  ...
  def test_remove_member_from_an_image(self):
     ...
     actual_response = self.images_client.delete_member_from_image(self.image_id, member_id)
  ...
  def test_replace_members_list_for_an_image(self):
     url = self.image_uri + '/members/'
     HTTPretty.register_uri(HTTPretty.PUT, url, body=self._build_members_list())
     
     actual_response = self.images_client.replace_members_list(self.image_id)
