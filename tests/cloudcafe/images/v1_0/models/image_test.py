...
class ImageTest(TestCase):
  ...
  def setUp(self):
    ...
    self.image_one = Image(
        ...
        deleted = False,
        container_format = "bare",
        created_at= datetime.strptime("2013-04-29T19:32:56", '%Y-%m-%dT%H:%M:%S'),
        disk_format = "qcow2",
        updated_at = datetime.strptime("2013-04-29T19:32:56", '%Y-%m-%dT%H:%M:%S'),
        owner = "bd7531a57d3a47538fae1b89c169b293",
        protected = False,
        min_ram = 0,
        checksum = None,
    ...
    self._dict = {'id': self.image_one.id,
  ...
  def test_positive_equality_of_images(self):
    assert self.image_one.__eq__(self.image_one)
    assert not self.image_one.__eq__(self.image_two)

  def test_negative_equality_of_images(self):
    assert self.image_one.__ne__(self.image_two)
    assert not self.image_one.__ne__(self.image_one)

  def test_dict_to_image(self):
    assert Image._dict_to_obj(self._dict).__eq__(self.image_one)

  def test_image_string_representations(self):
    regex = r'\[(\s?\w*\:\s(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}\.\d{6}|[\w\d\-\{\}]*)\,?)*\]'
  ...
  def test_json_to_image(self):
    ...
    assert Image._json_to_obj(self.raw_str)[0].__eq__(self.image_one)
