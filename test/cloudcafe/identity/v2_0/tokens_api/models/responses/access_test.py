...
class AccessTest(TestCase):
    def setUp(self):
        ...
        self.access_json_dict = open(os.path.join(os.path.dirname(__file__), "../../data/access.json")).read()
        ...
        self.expected_endpoint = Endpoint(admin_url=self.url, region=self.region,
                                          internal_url=self.internal_url,
                                          id_=self.id, public_url=self.public_url)

        self.expected_endpoint_list = EndpointList(endpoints=[self.expected_endpoint])
        ...
        self.expected_service = Service(endpoints=[self.expected_endpoint],
                                        endpoint_links=[], name=self.service_name,
                                        type_="TYPE")

        self.service_dict_list = [self.service_json_dict]
        self.expected_service_catalog = ServiceCatalog(services=[self.expected_service])
    ...
    def test_get_service(self):
        assert self.nova_service == self.access.get_service(name=self.service_name)
    ...
    def test_dict_to_obj(self):
        ...
        assert self.expected_service == Service._dict_to_obj(self.service_json_dict)
    ...
    def test_json_to_obj(self):
        assert self.expected_access == Access._json_to_obj(self.access_json_dict)
