...
from cloudcafe.common.behaviors import StatusProgressionError
...
from cloudcafe.blockstorage.volumes_api.v1.models import requests, responses
...
class wait_for_snapshot_status(unittest.TestCase):
    ...
    def get_mocks(self):
        ...
        snapshot_model = Mock(spec=responses.VolumeSnapshotResponse)
...
class wait_for_volume_status(unittest.TestCase):
    ...
    def get_mocks(self):
        client = Mock(spec=VolumesClient)
        config = Mock(spec=VolumesAPIConfig)
        volume_model = Mock(spec=responses.VolumeResponse)
        response = Mock(spec=Response)
...
class create_available_volume(unittest.TestCase):
    ...
    def get_mocks(self):
        client = Mock(spec=VolumesClient)
        volume_model = responses.VolumeResponse()
        volume_create_response = Mock(spec=Response)
        volume_model.id_ = "1111"
    ...
    @unittest.skip("Mock doesn't work with IsInstance anymore for some reason")
    def test_happy_path(self):
        ...
        print type(volume_entity)

        self.assertIsInstance(volume_entity, responses.VolumeResponse)
