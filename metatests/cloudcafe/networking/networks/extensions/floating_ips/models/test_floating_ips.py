...
class FloatingIPInfoListResponse(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Build the beginning of JSON list structure
        json_format_str = "{0}\n{1}"
        cls.json_response = '{{ "{root_tag}": ['.format(
            root_tag=cls.MODEL.ROOT_TAG)

        # Add each FLIP info JSON section to the structure
        for json_part in cls.json_parts:
            cls.json_response = json_format_str.format(
                cls.json_response, json_part)

            # Add a comma after each JSON section, except for the last element
            if json_part != cls.json_parts[-1]:
                cls.json_response = '{0},'.format(cls.json_response)

        # Close the overall JSON list structure
        cls.json_response = json_format_str.format(cls.json_response, ']\n}')
