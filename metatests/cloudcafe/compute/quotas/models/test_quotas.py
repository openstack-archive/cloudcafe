...
class QuotaDomainTest():
    ...
    def test_quota_attributes(self):
        assert str(self.quota.cores) == '20'
        assert str(self.quota.cores) == '20'
        assert str(self.quota.floating_ips) == '10'
        assert str(self.quota.id) == 'fake_tenant'
        assert str(self.quota.injected_file_content_bytes) == '10240'
        assert str(self.quota.injected_file_path_bytes) == '255'
        assert str(self.quota.injected_files) == '5'
        assert str(self.quota.instances) == '10'
        assert str(self.quota.key_pairs) == '100'
        assert str(self.quota.metadata_items) == '128'
        assert str(self.quota.ram) == '51200'
        assert str(self.quota.security_group_rules) == '20'
        assert str(self.quota.security_groups) == '10'
