...
class TestResult(unittest.TestCase):
    ...
    def test_failure(self):
        ...
        self.assertEqual(result, Result(False, {'test_failure': msg}))
...
class CheckEmailNotEmpty(PipelineTask):
    ...
    def task(data):
        if data['email']:
            ...
            return Result.failed('email was empty')
...
class CheckNameNotEmpty(PipelineTask):
    ...
    def task(data):
        if data['name']:
            ...
            return Result.failed('name was empty')


class TestPipelineMethods(unittest.TestCase):
    """Test methods used by Pipeline class"""

    good = Result.succeeded({'email': 'test@test.com'})
    bad = Result.failed('error')
    alt_bad = Result(False, {'alt_bad': 'bad'})
