...
from cloudcafe.designate.v2.quotas_api.models.requests import QuotasRequest
from cloudcafe.designate.v2.quotas_api.models.responses import QuotasResponse
...
class QuotasAPIClient(DesignateClient):
    ...
    def update_quotas(self, tenant_id=None, zones=None,
                      recordset_records=None, zone_records=None,
                      zone_recordsets=None, **requestslib_kwargs):
        """PATCH /quotas/{tenantID}"""
        quota_req = QuotasRequest(zones=zones,
                                  recordset_records=recordset_records,
                                  zone_records=zone_records,
                                  zone_recordsets=zone_recordsets)
        url = self._get_quota_url(tenant_id)
        return self.request('PATCH', url, response_entity_type=QuotasResponse,
