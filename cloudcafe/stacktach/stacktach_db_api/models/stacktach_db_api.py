...
class ImageUsage(AutoMarshallingModel):
    ...
    def __repr__(self):
        ...
        for prop, value in self.__dict__.items():
            values.append("{0}: {1}".format(prop, value))
        return "{0}: [{1}]".format(self.ROOT_TAG, ', '.join(values))
    ...
    def _json_to_obj(cls, serialized_str):
        ...
        image = cls._dict_to_obj(image_dict[cls.ROOT_TAG])
        return image
...
class ImageDelete(AutoMarshallingModel):
    ...
    def _json_to_obj(cls, serialized_str):
        """
        Returns an object of a ImageDelete based on the json serialized_str
        passed in.
        """
        json_dict = json.loads(serialized_str)
        delete = cls._dict_to_obj(json_dict[cls.ROOT_TAG])
        return delete
...
class ImageExist(AutoMarshallingModel):
    ...
    def _json_to_obj(cls, serialized_str):
        ...
        """
        json_dict = json.loads(serialized_str)
        exist = cls._dict_to_obj(json_dict[cls.ROOT_TAG])
        return exist
