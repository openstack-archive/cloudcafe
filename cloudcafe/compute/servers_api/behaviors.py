...
class ServerBehaviors(BaseBehavior):
    ...
    def ping_until_reachable(self, ip, timeout=None, interval_time=None):
        """
        @summary: Ping a server until it becomes reachable or a timeout
                  is reached
    ...
    def ping_until_unreachable(self, ip, timeout=None, interval_time=None):
        """
        @summary: Ping an active server until it becomes unreachable or a
                  timeout is reached
        @param ip: The IP address of the server
        @type ip: string
        @param timeout: The amount of time in seconds to wait before aborting.
        ...
        pingable = PingClient.ping(ip)

        if not pingable:
            raise Exception(
                "Server at {ip} was already unreachable".format(ip=ip))
