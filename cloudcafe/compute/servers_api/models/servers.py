...
class Console(AutoMarshallingModel):
    ...
    def _dict_to_obj(cls, dict):
        return Console(type=dict.get("type"),
                       url=dict.get("url"))
    ...
    @classmethod
    def _xml_ele_to_obj(cls, element):
        return Console(type=element.find("type").text,
                       url=element.find("url").text)


class ConsoleOutput(AutoMarshallingModel):
    ...
    def _json_to_obj(cls, serialized_str):
        json_dict = json.loads(serialized_str)
        return ConsoleOutput(output=json_dict.get("output"))

    @classmethod
    def _xml_to_obj(cls, serialized_str):
        element = ET.fromstring(serialized_str)
        cls._remove_xml_etree_namespace(element, Constants.XML_API_NAMESPACE)
        return ConsoleOutput(output=element.text)
