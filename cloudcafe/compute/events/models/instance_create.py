...
class InstanceCreateStart(EventBaseModel):
    """Compute Instance Create Start Response Model
    ...
    def __init__(self, access_ip_v4, access_ip_v6, architecture,
        ...
        super(InstanceCreateStart, self).__init__(locals())
...
class InstanceCreateError(EventBaseModel):
    """Compute Instance Create Error Response Model

    @summary: Response model for a compute.instance.create.error
        event notification
    @note: Represents a single event notification
    ...
    def __init__(self, access_ip_v4, access_ip_v6, architecture,
                 availability_zone, cell_name, code, created_at, deleted_at,
                 disk_gb, display_name, ephemeral_gb, exception, host,
                 hostname, image_meta, image_ref_url, instance_flavor_id,
                 instance_id, instance_type, instance_type_id, kernel_id,
                 launched_at, memory_mb, message, metadata, node, os_type,
                 progress, ramdisk_id, reservation_id, root_gb, state,
                 state_description, tenant_id, terminated_at, user_id, vcpus):
        super(InstanceCreateError, self).__init__(locals())

    @classmethod
    def _dict_to_obj(cls, json_dict):
        """Override dict_to_obj implementation"""
        obj = cls._map_values_to_kwargs(json_dict)
        obj.exception = InstanceException._dict_to_obj(obj.exception)
        obj.image_meta = ImageMeta._dict_to_obj(obj.image_meta)

        return obj


class InstanceCreateEnd(EventBaseModel):
    """Compute Instance Create End Response Model

    @summary: Response model for a compute.instance.create.end
        event notification
    @note: Represents a single event notification
