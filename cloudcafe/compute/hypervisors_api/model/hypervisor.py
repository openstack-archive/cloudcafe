...
class Hypervisor(AutoMarshallingModel):
    ...
    def _json_to_obj(cls, serialized_str):
        ...
        @return: Hypervisor or List of Hypervisors
        @rtype: Hypervisor or List
        """
        json_dict = json.loads(serialized_str)
        if 'hypervisor' in json_dict.keys():
            ret = cls._dict_to_obj(json_dict.get('hypervisor'))

        if 'hypervisors' in json_dict.keys():
            hypervisors = []
    ...
    def _xml_to_obj(cls, serialized_str):
        ...
        if element.tag == 'hypervisor':
            hypervisor_ele = element.find('hypervisor')
            hypervisor_dict = hypervisor_ele.attrib
            hypervisor_dict['service'] = hypervisor_ele.find('service').attrib
            ret = cls._dict_to_obj(hypervisor_dict)

        if element.tag == 'hypervisors':
            hypervisors = []
