...
class NetworkingBehaviors(NetworkingBaseBehaviors):
    ...
    def create_same_cell_n_servers(self, n_servers, network_ids,
                                   name='same_cell_server'):
        """
        @summary: Creates n servers in same cell using the first server ID with
            scheduler hints
        @param n_servers: number of servers to create
        @type n_servers: int
        @param network_ids: server networks, for ex. public, private, etc.
        @type network_ids: list
        ...
        for n_server in range(n_servers):
            ...
            if n_server < 1:
                ...
                resp = self.create_networking_server(
                    name=server_name, network_ids=network_ids,
                    scheduler_hints=scheduler_hints, active_server=False)
                server = resp.entity
        ...
        for created_server in created_servers:
            active_server = self.get_networking_server(created_server)
