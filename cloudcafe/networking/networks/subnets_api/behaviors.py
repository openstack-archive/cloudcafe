...
class SubnetsBehaviors(NetworkingBaseBehaviors):
    ...
    def get_ip(self, cidr, increment=None, decrement=None, randomize=True):
        ...
        @type cidr: string
        @param increment: places from the first IP of the CIDR to get the IP
        @type increment: int
        ...
        @param randomize: generate a random IP within the CIDR excluding first
            and last IPs, if increment and decrement are not given.
        @type randomize: bool
        @return: IP address
        @rtype: string
       """
    ...
    def get_ips(self, cidr, num=1):
        ...
        @rtype: list
        """
        ips = [self.get_ip(cidr) for x in range(num)]
        return ips
