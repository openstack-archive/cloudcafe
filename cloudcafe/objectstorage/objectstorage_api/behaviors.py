...
from cloudcafe.common.tools.dns_dig import dig_search
...
class ObjectStorageAPI_Behaviors(BaseBehavior):
    ...
    @staticmethod
    def generate_akamai_staging_urls(cdn_url, cdn_ssl_url):
        """
        @summary This method will generate urls to test against Akamai's
        staging network.

        @param cdn_url: The URL generated by CDN-enabling a container
        @type: string
        @param cdn_ssl_url: The SSL URL generated by CDN-enabling a container
        @type: string

        @return: A dictionary containing the generated urls and hosts for
                 Akamai's staging network
        @rtype: dictionary
        """

        # Get the dig results and clean them up
        akamai_cdn_dig_result = dig_search(cdn_url,
                                           search_string="edgesuite")
        akamai_cdn_dig_result = akamai_cdn_dig_result.replace('.net.',
                                                              '.net')
        akamai_cdn_dig_result = "http://" + akamai_cdn_dig_result

        # Create rigged CDN URL
        akamai_cdn_url = akamai_cdn_dig_result.replace(
            "edgesuite", "edgesuite-staging")

        # Create the rigged cdn url host
        akamai_cdn_host = cdn_url.replace("http://", "")

        akamai_staging_urls = {'akamai_cdn_url': akamai_cdn_url,
                               'akamai_cdn_host': akamai_cdn_host}

        # Get the dig results for ssl and clean them up
        akamai_cdn_ssl_dig_result = dig_search(cdn_ssl_url,
                                               search_string="edgekey")
        akamai_cdn_ssl_dig_result = akamai_cdn_ssl_dig_result.replace('.net.',
                                                                      '.net')
        akamai_cdn_ssl_dig_result = "https://" + akamai_cdn_ssl_dig_result

        # Create rigged CDN SSL URL
        akamai_cdn_ssl_url = akamai_cdn_ssl_dig_result.replace(
            "edgekey", "edgekey-staging")

        # Create the rigged cdn ssl url host
        akamai_cdn_ssl_host = cdn_ssl_url.replace("https://", "")

        akamai_staging_urls['akamai_cdn_ssl_url'] = akamai_cdn_ssl_url
        akamai_staging_urls['akamai_cdn_ssl_host'] = akamai_cdn_ssl_host

        return akamai_staging_urls
