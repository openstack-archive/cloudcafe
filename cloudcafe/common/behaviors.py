...
class StatusProgressionVerifier(BaseBehavior):
    ...
    def add_state(
            self, expected_status, timeout, poll_rate,
            max_poll_failure_retries=1, transient=False):
        ...
        self._state_list.append((
            expected_status, timeout, poll_rate, max_poll_failure_retries,
            transient))

    def start(self):
        #State loop
        current_state = 0
        for expected_status, timeout, poll_rate, max_poll_failure_retries, \
                transient in self._state_list:
            ...
            #Status-poll loop
            while time() < endtime:
                #Attempt to get the latest status
                try:
                    current_status = self.status_call(
                        *self.status_call_args, **self.status_call_kwargs)
                except Exception as exception:
                    if poll_failure_retries >= max_poll_failure_retries:
                        msg = "status_call() failed {retry_count} times, "
