    ...
    def _get_model_list(
        ...
        if not resp.ok:
            raise DatasetGeneratorError(
                ...
                "Request failed with an HTTP {0} ERROR".format(
                    model_type_name, resp.status_code))

        if resp.entity is None:
            raise DatasetGeneratorError(
                "Unable to retrieve list of {0} during data-driven-test setup"
                "Response: HTTP {0}".format(model_type_name, resp.status_code))
