...
class EventBaseModel(AutoMarshallingModel):
    ...
    def _map_values_to_kwargs(cls, deserialized_obj):
        ...
        for local_kw, deserialized_obj_kw in cls.kwarg_map.iteritems():
            kwargs[local_kw] = deserialized_obj[deserialized_obj_kw]
    ...
    def _validate_attributes(cls, deserialized_obj):
        ...
        missing_args = expected.difference(actual)
        cls._extra_args = actual.difference(expected)

        msg = ''
        if cls._extra_args and cls.strict_checking:
            msg += ' Found unexpected attributes: ' + str(list(cls._extra_args))
        if missing_args:
            msg += ' Missing expected attributes: ' + str(list(missing_args))
    ...
    def is_empty(self):
        ...
        for key in self.kwarg_map.keys():
            ...
            # Short-circuit check for empty primitive type
            if not value:
                continue
    ...
    def _validate_attributes(cls, json_dict):
        ...
        if len(json_dict) > 1:
            ...
            raise DeserializationError(
                'Unexpected attributes in deserialized string: ' +
                str(extras))
        if len(json_dict) < 1:
            raise DeserializationError('Missing list')
