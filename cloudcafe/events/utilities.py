...
class Result(object):
    ...
    Attributes:
        success (bool): True if the validation result was
            successful, False otherwise
        payload: Payload is different depending on the success
            variable. If success is True, payload is whatever
            input is being validated. If success is False, payload
            is a dict with keys mapping the failed validator name
            to the corresponding error message.

    Example:
        Result(True, 'A message to validate')
        Result(False, {'validator1': 'Validation 1 failed'})
    """
...
def bind(method):
    ...
    This decorator method adapts a validator method to work
    ...
    Args:
        method: A validator method that returns a Result

    Returns:
        function: Bound method that accepts and returns a Result
...
def process_sequential(validators, data):
    ...
    Args:
        validators (list): List of bound validators
...
def process_parallel(validators, data, reduce_method=validator_and):
    ...
    Args:
        ...
        reduce_method (function, optional): Method to reduce
            inputs to single Result. Defaults to validator_and.

    Returns:
        Result: Combined result of all validations
    """
    return reduce(reduce_method, [validator(data) for validator in validators])
