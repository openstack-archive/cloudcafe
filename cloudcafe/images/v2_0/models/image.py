...
class Image(AutoMarshallingModel):
    ...
    def __repr__(self):
        ...
        for prop in self.__dict__:
            if prop in ['created_at', 'updated_at']:
                ...
                values.append("%s: %s" % (prop, date_property.strftime('%Y-%m-%dT%H:%M:%SZ')))
            else:
    ...
    def _dict_to_obj(cls, json_dict):
        ...
        json_dict['visibility'] = getattr(ImageVisibility, json_dict['visibility'].upper())
        json_dict['container_format'] = getattr(ImageContainerFormat, json_dict['container_format'].upper())
        json_dict['disk_format'] = getattr(ImageDiskFormat, json_dict['disk_format'].upper())

        return Image(**json_dict)

    def _obj_to_json(self):
        obj_properties = deepcopy(self.__dict__)
        obj_properties['created_at'] = obj_properties['created_at'].strftime('%Y-%m-%dT%H:%M:%SZ')
        obj_properties['updated_at'] = obj_properties['updated_at'].strftime('%Y-%m-%dT%H:%M:%SZ')

        for key in ['id_', 'self_', 'file_']:
            obj_properties[key.rstrip('_')] = obj_properties[key]
            del(obj_properties[key])

        for key in ['status', 'visibility', 'container_format', 'disk_format']:
            obj_properties[key] = obj_properties[key].lower()

        return json.dumps(obj_properties)
