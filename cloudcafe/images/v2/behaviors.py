...
class ImagesBehaviors(BaseBehavior):
    ...
    def create_data_file(self):
        """@summary: Create object container and add object with data"""

        container_name = rand_name('container_name')
        object_name = rand_name('object_name')
        object_data = StringIO.StringIO(('*' * 10485760))
        CONTENT_TYPE_TEXT = 'text/plain; charset=UTF-8'
        headers = {'Content-Length': str(10485760),
                   'Content-Type': CONTENT_TYPE_TEXT}

        self.object_storage_behaviors.create_container(container_name)

        self.addCleanup(
            self.object_storage_client.force_delete_containers,
            [container_name])

        self.object_storage_client.create_object(
            container_name, object_name, headers=headers, data=object_data)

        response = self.object_storage_client.get_object(
            container_name, object_name)

        return response.entity.data_file
