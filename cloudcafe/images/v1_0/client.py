...
class ImagesClient(AutoMarshallingRestClient):
  ...
  def __init__(self, url, auth_token, serialize_format, deserialize_format):
    ...
    """
    super(ImagesClient, self).__init__(serialize_format, deserialize_format)
    self.auth_token = auth_token
    self.default_headers['X-Auth-Token'] = auth_token
    self.default_headers['Content-Type'] = ''.join(['application/', self.serialize_format])
    self.default_headers['Accept'] = ''.join(['application/', self.deserialize_format])
    self.url = url

  def list_images(self, **requestslib_kwargs):
  ...
  def filter_images_list(self, parameters_list, **requestslib_kwargs):
    url = '%s/images' % (self.url)
    return self.request('GET', url, params=parameters_list, 
            response_entity_type=Image, requestslib_kwargs=requestslib_kwargs)

  def retrieve_metadata(self, image_id, **requestslib_kwargs):
    url = '%s/images/%s' % (self.url,image_id)
    return self.request('HEAD', url, 
            requestslib_kwargs=requestslib_kwargs, )

  def retrieve_raw_image_data(self, image_id, **requestslib_kwargs):
    url = '%s/images/%s' % (self.url,image_id)
  ...
  def add_image(self, image_name, image_data, 
                headers=None,
                image_meta_id = None,
                image_meta_store = None,
                image_meta_disk_format = None,
                image_meta_container_format = None,
                image_meta_size = None,
                image_meta_checksum = None,
                image_meta_is_public = None,
                image_meta_min_ram = None,
                image_meta_min_disk = None,
                image_meta_owner = None,
                image_meta_property = None,
                **requestslib_kwargs):
    ...
    return self.request('POST', url, headers=headers,
      data=image_data, requestslib_kwargs=requestslib_kwargs)

  def list_image_membership(self, image_id, **requestslib_kwargs):
    url =  '%s/images/%s/members' % (self.url, image_id)

    return self.request('GET', url, requestslib_kwargs=requestslib_kwargs)

  def update_image(self, image_id, image_data=None,
                headers=None,
                image_meta_name=None,
                image_meta_store=None,
                image_meta_disk_format=None,
                image_meta_container_format=None,
                image_meta_size=None,
  ...
  def list_shared_images(self, member_id, **requestslib_kwargs):
      url = '%s/shared_images/%s' % (self.url, member_id)

      return self.request('GET', url, response_entity_type=Image, 
              requestslib_kwargs=requestslib_kwargs)

  def add_member_to_image(self, image_id, member_id, **requestslib_kwargs):
      url = ('%s/images/%s' + '/members/%s') % (self.url, image_id, member_id)
      return self.request('PUT', url, requestslib_kwargs=requestslib_kwargs)

  def delete_member_from_image(self, image_id, member_id, **requestslib_kwargs):
      url = ('%s/images/%s' + '/members/%s') % (self.url, image_id, member_id)
      return self.request('DELETE', url, requestslib_kwargs=requestslib_kwargs)

  def replace_members_list(self, image_id, **requestslib_kwargs):
    url = ('%s/images/%s' + '/members/') % (self.url, image_id)
    return self.request('PUT', url, requestslib_kwargs=requestslib_kwargs)
