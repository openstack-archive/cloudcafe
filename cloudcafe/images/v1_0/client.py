...
class ImagesClient(AutoMarshallingRestClient):
    ...
    def __init__(self, url, auth_token, serialize_format, deserialize_format):
        ...
        self.default_headers['Content-Type'] = ''.join(['application/',
                                                        self.serialize_format])
        self.default_headers['Accept'] = ''.join(['application/',
    ...
    def list_shared_images(self, member_id, **requestslib_kwargs):
        url = '%s/shared-images/%s' % (self.url, member_id)

        return self.request('GET', url, response_entity_type=Image,
                            requestslib_kwargs=requestslib_kwargs)

    def add_member_to_image(self, image_id, member_id, **requestslib_kwargs):
        url = ('%s/images/%s' + '/members/%s') % (self.url, image_id,
                                                  member_id)
